...
<main class="relative flex flex-col w-full max-w-[1280px] px-[75px] mx-auto mt-[50px] mb-[62px]">
    ...
    <div class="flex gap-[30px] mt-[30px]">
        ...
        <!-- Form Tiers -->
        <form action="{{ route('booking', $flight->flight_number) }}" id="Tiers" method="POST" class="grid grid-cols-2 gap-x-[30px] flex-1">
            @csrf
            ...
            @foreach ($flight->classes as $class)
                <div class="flex flex-col h-fit rounded-[20px] p-5 pb-[30px] gap-5 bg-white">
                    <div class="w-[260px] h-[180px] rounded-[30px] bg-[#D9D9D9] overflow-hidden mx-auto">
                        @if ($class->class_type === 'economy')
                            <img src="{{ asset('assets/images/thumbnails/economy-seat.png') }}" 
                                class="w-full h-full object-cover" alt="Economy seat thumbnail">
                        @else
                            <img src="{{ asset('assets/images/thumbnails/business-seat.png') }}" 
                                class="w-full h-full object-cover" alt="Business seat thumbnail">
                        @endif
                    </div>
                    ...
                    @foreach ($class->facilities as $facility)
                        <div class="flex items-center gap-[10px]">
                            <img src="{{ asset('storage/' . $facility->image) }}" class="w-6 h-6 flex shrink-0" alt="baggage icon">
                            <p class="font-semibold">{{ $facility->name }}</p>
                        </div>
                    @endforeach
                    
                    <!-- Button - href dihapus -->
                    <button type="button" class="w-full rounded-full py-3 px-5 text-center bg-garuda-blue hover:shadow-[0px_14px_30px_0px_#0068FF66] transition-all duration-300 block mt-auto"
