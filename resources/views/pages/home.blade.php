...
@section('content')
     <div id="Hero-Text" class="relative flex flex-col w-full max-w-[1280px] px-[75px] mx-auto gap-[30px] mt-[86px]">
        ...
        <h1 class="font-extrabold text-[50px] leading-[75px]">Fly Further<br>Fly Comfortably with Garuda</h1>
        <p class="text-lg leading-8">Find the best flights at affordable prices with premium service for every journey.</p>
    </div>
                        ...
                        class="dropdown-content hidden absolute z-10 top-full mt-4 h-[232px] rounded-[18px] bg-white border border-[#E8EFF7] overflow-y-scroll custom-scrollbar">
                            <div class="flex flex-col justify-center w-[483px] p-5 gap-4 shrink-0">
                                @foreach ($airports as $airport)
                                    ...
                                    <input type="radio" name="departure" id="departure-{{ $airport->iata_code }}" 
                    ...
                    <div id="Arrival" 
                        ...
                        <div id="Arrival-Dropdown" class="dropdown-content hidden absolute z-10 top-full mt-4 h-[232px] rounded-[18px] bg-white border border-[#E8EFF7] overflow-y-scroll custom-scrollbar">
                            <div class="flex flex-col justify-center w-[483px] p-5 gap-4 shrink-0">
                               @foreach ($airports as $airport)
                                    <label class="relative flex items-center rounded-[10px] gap-[10px] p-0 has-[:checked]:p-[10px] has-[:checked]:bg-garuda-bg-grey transition-all duration-300">
                                    <input type="radio" name="arrival" id="arrival-{{ $airport->iata_code }}" 
                                    class="absolute top-1/2 left-1/2 opacity-0" value="{{ $airport->iata_code }}">
                                    <img src="assets/images/icons/airplane-black.svg" class="flex shrink-0 w-[34px]" alt="icon">
                                    <div class="flex flex-col gap-[2px]">
                                        <p class="font-semibold">{{ $airport->name }} ({{ $airport->iata_code }})</p>
    ...
    <section id="Popular" class="relative flex flex-col gap-[30px] mt-[70px] mb-[86px]">
        <div class="w-full max-w-[1280px] px-[75px] mx-auto">
            <h2 class="font-bold text-[28px] leading-[42px]">Most Popular Destinations</h2>
            <p class="text-lg mt-[6px]">Discover the most sought-after destinations loved by our travelers.</p>
        </div>
        <div class="swiper !w-full overflow-x-hidden">
          ...
          <div class="swiper-slide !w-fit first:ml-[calc(((100%-1280px)/2)+75px-24px)]">
                    <a href="#" class="card">
                        <div class="relative flex items-end w-[230px] h-[280px] shrink-0 rounded-[30px] bg-white overflow-hidden hover:border-2 hover:border-garuda-blue hover:p-[10px] transition-all duration-300">
